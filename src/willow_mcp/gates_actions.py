...
def describe(row: GateRow) -> ActionSpec:
    ...
    if rid.startswith("request."):
        ...
        return ActionSpec(
            kind="none",
            reason=f"{row.scope} cannot be requested — a request may activate a "
                   f"standing grant, never create one",
        )
...
def _request_grant(row: GateRow, inputs: dict) -> dict:
    ...
    if left is not None and left <= 0:
        return {"ok": False,
                "message": f"request {item_id} expired at {req.get('expires_at')}; "
                           f"dismiss it and ask again rather than granting late"}

...
TOOL_CLASS: dict[str, str] = {
    ...
    # Brokered systemd --user install/reload (kart-vs-operator partition):
    # never Kart; EXECUTE under willow-gate when the app is registered.
    "unit_install_execute": EXECUTE, "unit_reload_execute": EXECUTE,
    "unit_ops_status": READ,
    # ── admin (never sudo) ────────────────────────────────────────────────────

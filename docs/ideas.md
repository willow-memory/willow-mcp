...
>
> **Cross-pollination pass (2026-09-21, fork-audit-85x3n5):** graded ~13
> deliberately off-domain `rudi193-cmd` forks for *transplantable mechanisms*
> rather than drop/adopt — the misfits, on the theory that novel shapes come
> from unexpected domains. Added #76–83 (in Cross-cutting), each tagged with its
> target subsystem and the source fork. Notable convergence: three unrelated
> forks (sigmap, mealie, glapagos) independently argued for weighting
> Mem-ratify's quorum (#77). Considered and left out: NaturalConv (a clean
> grounded/ungrounded dialogue corpus for friction-floor *eval*, but
> non-commercially licensed — mirror the shape, don't vendor it). Seeds, not
> landings; untagged because nothing is built yet.
...
> *Cross-pollination run (2026-09-21):* #76–83 below came from mining off-domain
> `rudi193-cmd` forks for transplantable mechanisms. Each names its target
> subsystem and source fork. Seeds, not landings.

76. **🌱 Two-stage tool routing + dispatch token budgets** — route by capability *category* first, then expose only that category's tool schemas; enforce a per-agent-per-task token budget at the dispatch boundary with escalation (summarize, or drop to a smaller model) instead of overflowing context. → Nestor routing + Kart. *(fork: smallcode — category→schema routing + context-budget eviction.)*
77. **🌱 Weighted epistemic quorum** — Mem-ratify tier promotion (Contested→Frontier→Canonical) weights each witness vote by domain relevance and provenance share instead of counting votes uniformly. → Mem-ratify / governance. *(convergence — three forks independently: sigmap TF-IDF relevance weighting, mealie proportional-dependency aggregation, glapagos stakeholder councils.)*
78. **🌱 Render-once, view-many atoms** — store a KB atom once; render it through swappable MarkdownAI directive templates ("summarize lineage", "show hierarchy", "list dependents") rather than duplicating data per view. → KB atoms + MarkdownAI. *(fork: CSL text2text prompt-template system over one dataset.)*
79. **🌱 Plan / validate / BLOCKED before execute** — a terraform-plan-style dry run for willow-compose: pre-flight each service (config valid? state dirty?), emit CREATE/UPDATE/UNCHANGED/BLOCKED/ERROR, and refuse to deploy on BLOCKED instead of failing mid-apply. → willow-compose. *(fork: vcspull WorktreeCheck → WorktreeAction → aggregated result.)*
80. **🌱 Boundary-preserving atom transforms** — rewrite an atom's payload (redact lineage, promote a tier) while holding its semantic-zone boundaries and cross-references fixed, so graph topology never breaks under a content edit. → Nest content wall / KB atoms. *(fork: PDFMathTranslate region-aware layout-preserving substitution.)*
81. **🌱 Cross-source atom entity resolution** — when two atoms from different sources share concept/lineage overlap, dedupe-merge them with a confidence score and kept provenance — atom-level, beyond intake near-text dedup (#20). → KB atoms graph. *(fork: ai-job-search cross-portal `seen_jobs` unification.)*
82. **🌱 Composite tiered health score** — give tree-view a normalized weighted health number per subtree (staleness, lineage-break rate, verify-fail rate → 0–100 → Healthy/Degraded/Critical) instead of a qualitative read; feeds #70. → tree-view health. *(fork: HeatWatch multi-factor normalized composite index.)*
83. **🌱 Knowledge-edge integrity check** — extend `knowledge_verify` to flag `knowledge_edge`s with broken or incomplete `lineage_*` chains (dangling references) — a referential-integrity sweep over the graph. → knowledge_verify / lineage. *(fork: mex drift-detection CLI validating path/ref validity.)*

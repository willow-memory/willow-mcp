...
def test_manifest_compiler_carries_explicit_aliases():
    ...
    assert manifest["collection_aliases"]["pm/portfolio"] == ("projects_willow_pm_portfolio")


def test_aliases_resolve_explicitly_and_canonical_names_remain_available(tmp_path, monkeypatch):
    _manifest(tmp_path, monkeypatch)
    assert gate.resolve_collection_alias("willow", "pm/portfolio") == (
        "projects_willow_pm_portfolio",
        None,
    )
    assert gate.resolve_collection_alias("willow", "projects_willow_pm_portfolio") == (
        "projects_willow_pm_portfolio",
        None,
    )
...
def test_project_handoffs_do_not_bleed(tmp_path, monkeypatch):
    monkeypatch.setenv("WILLOW_HOME", str(tmp_path))
    dispatch.session_handoff_write("hanuman", "s1", narrative="alpha only", project="alpha")
    dispatch.session_handoff_write("hanuman", "s2", narrative="beta only", project="beta")
...
def test_specialist_dispatch_session_receives_project_orientation(tmp_path, monkeypatch):
    _manifest(tmp_path, monkeypatch, app="hanuman")
    monkeypatch.setattr(server, "_store", Store(tmp_path / "store"))
    sent = dispatch.dispatch_send("willow", "hanuman", "# Build\n\nDo the work.", summary="build")
...
def test_session_start_bridge_invokes_session_enter_without_legacy_hook(monkeypatch):
    seen = {}
...
def test_cursor_hook_uses_a_stable_available_interpreter():
    ...
    # Post-2026-09-13 PR 2b: SessionStart routes through the shared runner
    # instead of `willow_mcp.session_start_hook` directly. Accept either
    # shape so this test survives a re-migration.
    assert "willow_mcp.hook_runner" in cmd or "willow_mcp.session_start_hook" in cmd
    if "willow_mcp.hook_runner" in cmd:
...
def test_project_identity_is_collision_safe_across_workspaces(tmp_path):
    a = tmp_path / "a" / "charter"
    a.mkdir(parents=True)
    b = tmp_path / "b" / "charter"
